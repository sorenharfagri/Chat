...
const InviteButton = ({room}) => {
  ...
  const url = window.location.hostname+":3000/chat?room="+room; //Ссылка для приглашения

  return (
    <Button className="child-button" variant="light" onClick={ () => setShow(!isShow) }> Invite friend {/* //При нажатии на кнопку отображаем окно */}
      {isShow ? <> {/* Если кнопка была нажата - меняется статус, и отображается всплывающее окно */}
